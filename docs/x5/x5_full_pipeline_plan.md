...
### 三项优化跑完的实测（同包、同 442 关键帧）

| | ① 3cm/1度/1s 原代码 | ② 10cm/3度/5s | **③ ② ＋ 跳彩色 ＋ TF 门** |
|---|---|---|---|
| 关键帧 | 837 | 442 | **442**（与②逐个相同） |
| **墙上时间** | 942 s (15.7 min) | 865 s (14.4 min) | **406 s (6.77 min)** |
| 对①的加速 | — | 1.09x | **2.32x** |
| 地图体积 | 1837 MB | 971 MB | **971 MB**（与②差 78 字节） |
| 错误 | 0 | 0 | 0 |
| `mapping_loop` | 220.9 ms | 222.4 ms | 219.5 ms |
| `msg_decode` | 8.9 ms | 8.6 ms | **4.4 ms**（少了 rgb 的 cv_bridge 转换） |
| `tf_publish` | 16.8 s | 9.0 s | **0**（已被门掉） |
| 未被任何计时段覆盖 | 83.5% | 83.5% | **69.5%** |

**彩色那一刀是主力**：播放阶段每 1% 的耗时从 8.20 s 降到 3.70 s。它值这么多不只是因为占 32%
的消息数 —— 每帧还要在那个被占满的单线程里做一次 JPEG 解码和一次约 1 MB 的原始图重发。

⚠️ **两个数变了但不该记在这次改动头上**，原因未查明，别当成收益：
`feature_extractor` 179.2 -> **196.8 ms（慢了 10%）**，`occupancy_raycast` 22.3 -> **13.1 s**。
后者的候选解释是页缓存（播放阶段 I/O 少了，读 `depths.dat` 时缓存更充裕），前者可能是温度或
BPU 队列，都没验证。
