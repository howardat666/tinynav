...
**当前状态一句话**：Phase 0f（新机基线）已完成；64GB 机器的 MIPI 故障已**定位为软件问题**（sensor 被写成 1 lane / host 配成 2 lane，旧 OTA 残留的 `libsc132gs.so` 只有 1-lane 寄存器表）—— **不是排线坏**，1 lane 下两路立体相机都能跑满 62 fps。PC 侧工作全部不受阻塞。
