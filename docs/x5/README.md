...
**当前状态一句话**：Phase 0 已完成 0a / 0b（经典部分）/ 0e / 0f。⭐ **最重要的结论：白天场景下经典路线（ORB + 自训练 DBoW3）R@1 90.1%，已经超过 DINOv2 patch VLAD 的 88.87%，而且快 4 倍、省 2 倍内存 —— 第一版直接上方案 1，不用等 BPU。** 🔴 **夜间所有方法都不可用**（最好 R@1 21.6%），机理是夜间 ORB 特征点从 916 掉到 333 个/帧，**换描述子救不了，得靠补光或双时段地图**。🟢 BPU 已实测可被第二个进程并发使用；64GB 机器的 MIPI 故障定位为**旧 OTA 残留的 sensor 库只有 1-lane 寄存器表**（**不是排线坏**）。剩下 0c/0d，以及 0b 的学习式部分 —— **共同卡在 PC 的 NVIDIA 内核模块与运行内核不匹配**。
...
| 方案 | 组合 | 单次用时<br>(4.5 核) | 内存 | Day R@1 | 状态 |
|---|---|---|---|---|---|
| **1 全经典** | **自训练 DBoW3** + ORB + BF-Hamming | ⬤ **0.38 s** | ⬤ **~160 MB** | ⬤ **90.1%** | ⭐ **第一版就做这个（= PR #136）** |
| 5a | BoW + SuperPoint + BF-L2 | ⬤ **1.50 s** | ~280 MB | 🟡 未测 | ✅ 退路 |
| **6** | DINOv2@BPU + SuperPoint + BF-L2 | ○ **1.58 s** | ○ **~350 MB** | 88.87%（fp16）| 🟡 长期目标（🟢 BPU 并发已验证）|
| ~~0 / 2 / 3 / 4~~ | 含 LightGlue 的方案 | 4.5–20 s | — | — | ❌ **5s 预算判死** |

**四条已确立的判断**：

1. ⭐ **白天场景下，经典路线已经打平甚至超过学习式** —— 自训练 1 万词 DBoW3 的 Day R@1 **90.1%**，高于 DINOv2 patch VLAD 的 88.87%，而它零神经网络、快 4 倍、省 2 倍内存。**方案 6 的 recall 优势只剩夜间。**
2. 🔴 **夜间谁都不行，而且不是换方案能解决的** —— 最好的也只有 R@1 21.6%（DINOv2 31.2%）。机理是**夜间 ORB 特征点从 916 掉到 333 个/帧**，失败发生在特征提取而非检索。出路只有补光或双时段地图。
3. **不要试图加速 LightGlue，而是不用它** —— cv2 BF 匹配 38.6 ms 就能替代它的 2870 ms。它同时是 CPU 上最慢的（占 57% 总耗时）和最难上 BPU 的（Einsum×36 + IsInf/IsNaN×60）。
4. **上 BPU 不省内存** —— X5 的 BPU 与 CPU 共用同一块 DDR，没有独立显存。省内存靠换运行时（ORT → hbDNN），不靠上 BPU。

⚠️ 还有四项 recall 没数据（DINOv2 int8 / SuperPoint BoW / SP+BF / DINOv2 在同一套地图上复测），**共同卡在 PC 的 GPU 驱动上**。详见 [`x5.md § 10.4`](x5.md)。
...
| **0b** | 给 `tool/benchmark/map_retrieval_self_consistency.py` 加描述子后端开关，跑齐 recall 空白 | PC | 1–2 天 | 🔴 **所有方案的 recall 生死线** | 🟢 **经典两项已完成 —— 方案 1 白天打平学习式**，见 [`x5.md § 10.2`](x5.md)；学习式那几项卡 GPU 驱动 |
| **0c** | `hb_mapper` 把 DINOv2 编成 int8 `.bin` | PC + OpenExplorer | 1–3 天 | 🔴 0b 剩余项的前置 | 🔴 **卡住** —— PC 的 NVIDIA 模块与运行内核不匹配 |
| **0d** | **办公室日夜采数**：同一路线录三趟 bag（白天 / 晚上开灯 / 晚上关灯），肉眼看图 + 跑 0b | 办公室 | 1 天 | 🔴 **日夜方案生死 + 是否红外** | 🟡 需一台能用的 Looper。⚠️ 现在更要紧了：0b 已证明**夜间所有方法都不可用**，得先确认办公室夜间到底什么光照 |
