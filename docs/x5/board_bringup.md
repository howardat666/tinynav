...
### 🟢 日常跑导航：不用开终端（2026-08-24 起）

板子一上电，`board-app.service` 会在时钟对好之后自己把 app 起来（`After=board-autotime`
不是可省的：早于它就把整份 `app.log` 写成 2025-08-26，事后没法和 `board_health.log` 对时间）。
所以正常流程只有三步，全在浏览器里：

1. 打开 `http://192.168.19.218:8000/`
2. **Device 页 → Run logs → Start collecting**
3. 建图 / 导航照常在 web 上点；跑完 **Stop collecting**，再点包旁边的下载图标存到 PC

换方案（a/b/c）：写 `/userdata/x5/app.env` 一行 `TINYNAV_SCHEME=b`，然后
`systemctl restart board-app`。默认是 `a`（map=vio, nav=vio）。

Start collecting **不启动任何写日志的东西** —— 日志本来就一直在写。它记的是几个偏移量，
Stop 的时候只切出这一段。所以按不按它都不影响系统行为，忘了按最多是包里多一点东西。

包里有什么、以及**哪些节点这次一个字都没写**（`run.json` 的 `nodesSilent`）都是显式列出来的。
这一条是被坑出来的：以前打包会在某个节点没日志时塞一份**上一次 run 的**同名文件进去，
2026-08-24 那次就把一份 08-18 的 `wheel_odometry.txt` 当成当天的读了，诊断整个跑偏。

> `app.log` 现在每次启动轮转一代（`app.log.1`）。以前它只追加，涨到 44000 行横跨一年。
...
### 2026-08-24 掉线定案:射频干扰,不是掉压(我先错判成掉压)
...
WiFi、ESP32、Looper、电机确实**全部由同一个电池供电**,所以「电机电流拉塌共用轨打死网卡」
看起来顺理成章 —— 我据此下了结论。**但当天第二次掉线的仪表数据把它否掉了,见下面一节。**
留着这段是为了记住这个错判:塌压和掉线都发生在"我们正在开车"的时候,**相关不等于因果**。
...
### 🔴 12:06 第二次掉线:`usb=`/`carrier=` 一上线就把掉压假设否掉了

上午刚把 `board_health` 改成双写并加了 `carrier=`/`usb=`,同一天 12:06 又掉一次 —— 这次有数据:

```
12:04:56 ... rssi=-54 qual=93 link=1 fa=593  rx_rate=MCS7   carrier=1 usb=1 pc=56
12:06:27 ... rssi=-51 qual=90 link=1 fa=353  rx_rate=MCS7   carrier=1 usb=1 pc=21
12:06:37 ... rssi=-60 qual=68 link=1 fa=2309 rx_rate=MCS4   carrier=1 usb=1 pc=ERR   <- 阶跃
12:07:38 ... rssi=-55 qual=94 link=1 fa=1659 rx_rate=CCK_1M carrier=1 usb=1 pc=ERR
```

🔑 **`usb=1 carrier=1 link=1` 全程不变** —— 网卡从没离开 USB 总线,association 也没断。
**不是掉电。**而且顺序也不对:

```
12:06:37  board_health: fa 353 -> 2309，pc=ERR        <- 网络在这一刻死
12:06:38  diffcar_control: PC unreachable，青灯
12:06:50  battery sagged to 9.58 V (瞬时 11.04)       <- 掉压在 12 秒之后
12:06:52  battery back to 11.20 V
```
...
| | 正常 (n=492) | 故障 (n=42) |
|---|---|---|
| `fa` 误报警 | p50 **377**,p90 577 | p50 **1136**,p90 **2271** |
| `load` | p50 5.00,**max 9.91** | p50 6.14,max 10.50 |
| `temp` | p50 89.5,**max 91.6** | p50 89.5,max **90.7** |

**温度在故障期反而更低,而 `load` 9.18~9.91 在正常期出现过 6 次** —— 两个都不成立。而 `fa` 是
**阶跃**跳上去的,和 `pc=ERR` 同一个采样,`rssi` 没变(−51 → −60,和平时的波动同量级)。
误报警是"接收机被不能解码的能量触发"的计数,**rssi 不变而它涨 7 倍 = 噪声底上来了,不是路径损耗**。

⚠️ **`rx_rate` 崩到 `CCK_1M` 晚了约 50 秒,所以速率崩溃是结果不是原因。**单点出现 `CCK_1M`
是常态(它是"最后一帧的接收速率"),**只有持续段才算判据**。

🔴 **「电机噪声干扰」也被否掉了**:`load` 在 12:00–12:06 已经 8~9.91(车一直在动),而那段
`fa` 完全正常(150~790)。**开车本身不会把 `fa` 顶起来。**所以它是一个**外来的**干扰事件。

**能修的地方在"为什么不自愈"**:link / carrier / usb 全程都在,说明只要**踢一下接口**大概就能
回来,不需要跑过去拔电。这类 Realtek USB 驱动卡在最低速率不出来是已知毛病。升级路径应该是
`ip link down/up` → 重新 association → `/sys/bus/usb/devices/1-1/authorized` 重新枚举 → 才是重启。

⚠️ `pc=ERR` 现在会带 errno(`ERR105`=ENOBUFS 发送队列堵死 / `ERR101`,`ERR113`=路由或主机不可达)。
链路还在而只有这里报错,最可能是 1 Mbit/s 下发送队列被挤爆 —— 那样连 ARP 都发不出去,
从外面看就是"二层都没响应"(实测 `arp -n` 是 `incomplete`)。**下次掉线看这个 errno 就能定案。**
