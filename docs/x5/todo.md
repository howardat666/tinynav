...
> ## ⚠️ 这份清单停在 2026-08-04，大部分「下一步」已经做完或已过时
>
> 08-05 → 08-14 实际做了什么、以及当前真正的待办，见
> **[`nav_field_results.md`](nav_field_results.md)**（§5 已完成、§6 诊断出但未修、§7 方向变更）。
>
> 当前排在最前的四项（都在 `nav_field_results.md` §6 里有依据）：
>
> | 优先 | 事项 | 阻塞 |
> |---|---|---|
> | 1 | 🔴 **`map_node` 被 OOM 杀掉**（`rc=-9`）—— 内存去向没算清 | **SuperPoint 上板的硬阻塞** |
> | 2 | 夜间重定位：检索命中只有 26–32%，是天花板 | 需要 SP 进检索层，或双时段地图 |
> | 3 | 速度只有指令的 26%（舵机扭矩 vs 里程计标度，二选一）| **需要机器人**，卷尺量 2 m 计时 |
> | 4 | `T_map_odom` 的 100 样本窗口 → 改按时间开窗 + 加「重新定位」控件 | 先测重定位噪声散布 |
