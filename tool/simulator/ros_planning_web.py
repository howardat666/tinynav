...
# x5 分支把上游的 robot_specs 叫 robot_config，且默认平台是差速车不是 go2。
...
def _robot_dict(name: str | None = None) -> dict[str, Any]:
    key = (name or DEFAULT_ROBOT.name).strip().lower()
    return copy.deepcopy(ROBOT_PRESETS.get(key, asdict(DEFAULT_ROBOT)))
...
SIM_POSE_TOPIC = "/slam/pose_sim"


def pose_from_T(T: np.ndarray, stamp, frame_id: str = "world") -> PoseStamped:
    """x5 的 planning_node / cmd_vel_control 闭环用的是 PoseStamped，不是 Odometry。"""
    ...
    (msg.pose.orientation.x, msg.pose.orientation.y,
     msg.pose.orientation.z, msg.pose.orientation.w) = map(float, quat)
    return msg
...
class RosPlanningSimNode(Node):
    def __init__(self):
        ...
        self.pose_pub = self.create_publisher(PoseStamped, SIM_POSE_TOPIC, 10)
        self.camera_info_pub = self.create_publisher(CameraInfo, "/camera/camera/infra2/camera_info", 10)
        latched = QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL)
        # 必须 latched：x5 的 planning_node 用 TRANSIENT_LOCAL 订阅这个话题，而
    ...
    def tick(self) -> None:
        ...
        # 和 depth 同一个 stamp —— planning 那边是精确时间戳同步，差一纳秒就零回调
...
def _active_robot_type() -> str:
    if SIM_NODE is None:
        return DEFAULT_ROBOT.name
    return str(SIM_NODE.config.get("robot", {}).get("name", DEFAULT_ROBOT.name)).strip().lower()
...
def _spawn_if_needed(script: str) -> None:
    ...
    if any(script in " ".join(p.args) for p in PROCS):
        return
    # 用跑本进程的那个解释器，而不是写死 `uv run` —— 板上/镜像里的系统 python 已经带齐
    # 依赖，而 `uv run` 会在挂载进来的仓库里现建一个几 GB 的 venv。
    argv = [os.environ.get("TINYNAV_SIM_PYTHON", sys.executable), script, "--ros-args",
            "-p", f"robot_type:={_active_robot_type()}",
            "-p", f"pose_topic:={SIM_POSE_TOPIC}",
            "-p", "pose_sync:=exact"]
    PROCS.append(subprocess.Popen(argv, cwd=str(REPO_ROOT), env=_child_env()))
